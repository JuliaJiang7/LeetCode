...
public class Solution98 {
    ...
    public boolean isValidBST2(TreeNode root){
        long maxValue = (long) Integer.MAX_VALUE + 1;  //节点值均为 Int 型，故最大值设为 long
        long minValue = (long) Integer.MIN_VALUE - 1;
        return getAnswer2(root, minValue, maxValue);
    }

    private boolean getAnswer2(TreeNode root, long minValue, long maxValue){
        if(root == null)
            return true;
        //注意：等于时要返回false
        ...
        if(root.left != null){
            isValidLeft = getAnswer2(root.left, minValue, root.val);
        }
        if(!isValidLeft)    return false;

        boolean isValidRight = true;
        if(root.right != null){
            isValidRight = getAnswer2(root.right, root.val, maxValue);
        }
        return isValidRight;
    }
